...
class SearchPanel extends React.Component {
  ...
  searchInputValueChange = (event) => {
    const gamesToDisplay = event.target.value
      ? this.props.userBlocks.filter((block) => {
  ...
  render() {
    const gamesToRender = this.state.foundGames
      .map((elem) => {
    ...
    const results = <div className="search-matrix">{gamesToRender}</div>;

    return (
      <div className="search-panel-wrapper">
        {panel}
        {this.state.foundGames.length ? (
          <span
