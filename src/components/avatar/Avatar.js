...
const Avatar = ({ onImageLoad, src, userData }) => {
    ...
    const uploadChange = (event) => {
        if (onImageLoad) {
            ...
            fileDataReader.addEventListener('load', function () {
    ...
    return (
        <span
            ...
            <span className={'profile-avatar-overlay'}>
                <span className={'profile-avatar-overlay-icon'}>
                    {CameraIcon}
                </span>
            </span>
            {source === 'https://u.o0bc.com/avatars/stock/_no-user-image.gif' ? "" : deleteButton}
        </span>
    )
};
