...
class Dashboard extends React.Component {
  ...
  render() {
    const listsToRender = this.props.userLists.map((elem, index) => {
      return (
        <NavLink key={elem.id} to={"/lists/" + elem.id}>
          <ListBlock listBlockIndex={index} />
        </NavLink>
      );
    });
    ...
    const addListButton = (
      <AddButton
        ...
        additionalClass={"gk-add-button-dashboard"}
      />
    );

    const addListNode = this.state.showAddListTool ? (
      <AddListTool
    ...
    const authorized = (
      <div className="lt-col dashboard-elements">
        <div className="lists-dashboard-wrapper lt-row">
          {listsToRender}
          {this.props.userData && this.props.userData.email === DemoUser
            ? ""
            : addListNode}
        </div>
      </div>
