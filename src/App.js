...
class App extends React.Component {
	...
	fetchAvatar() {
		...
		ref
			.child(`avatars/${this.props.userData.uid}`)
			.getDownloadURL()
			.then((url) => {
				this.setState({
					avatarSrc: url,
			...
			>
				<BrowserRouter>
					{(this.state.listsSectionsBlocksLoaded &&
						...
						this.state.platformsDataLoaded) ||
...
const appDispatchToProps = (dispatch) => {
	return {
		...
		fetchColors: (snapshot) => {
			dispatch({
				type: colorsActions.actions.COLORS_FETCH,
				snapshot: snapshot,
			});
		},
		fetchPlatforms: (snapshot) => {
			dispatch({
				type: platformsActions.actions.PLATFORMS_FETCH,
				snapshot: snapshot,
			});
		},
		...
		setSectionsToStore: (sections) => {
			dispatch({
				type: userSectionsReducer.actions.SECTIONS_SET,
				sections: sections,
			});
		},
